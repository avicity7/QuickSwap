...
def buyTokens(tokenA,tokenB,personal_wallet,private,min_amount):
    pancakeswap2_txn = contract.functions.swapExactETHForTokens(
        ...
        'value': amount,#This is the Token(BNB) amount you want to Swap from
        'gas': 250000,
        'gasPrice': w3.toWei('10','gwei'),
        'nonce': nonce,})
    signed_txn = w3.eth.account.sign_transaction(pancakeswap2_txn, private_key=private)
    tx_token = w3.eth.send_raw_transaction(signed_txn.rawTransaction)
    if tx_token != "": 
        print("Success")
        print("Txn hash: " + w3.toHex(tx_token))
    else: 
        print('Failed')
...
while True: 
    ...
    tokenA = w3.toChecksumAddress('0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c')
    tokenB = input("Enter the contract address of the token you're buying: ")
    if tokenB != "logout": tokenB = w3.toChecksumAddress(tokenB)
    else: break
    amount = w3.toWei(input("Enter the amount of BNB used to buy: "),"ether")
    path = [tokenA, tokenB]
    ...
    if purchaseMethod == 2: 
        while True:
            ...
            CAKE_ROUTER_V2 = w3.toChecksumAddress('0x10ed43c718714eb63d5aa57b78b54704e256024e')
            ...
            def get_price(token, decimals, pair_contract, is_reversed, is_price_in_peg):
                ...
                return Decimal('0')
            CAKE_FACTORY_V2 = w3.eth.contract(address=CAKE_ROUTER_V2, abi=ABI).functions.factory().call()
            try:
                token = w3.toChecksumAddress(tokenB)
                pair = w3.eth.contract(address=CAKE_FACTORY_V2, abi=ABI).functions.getPair(token, WBNB).call()
                pair_contract = w3.eth.contract(address=pair, abi=ABI)
                is_reversed = pair_contract.functions.token0().call() == WBNB
                decimals = w3.eth.contract(address=token, abi=ABI).functions.decimals().call()
                is_price_in_peg = True
                liquidityPresence = float(get_price(token, decimals, pair_contract, is_reversed, is_price_in_peg))

                if liquidityPresence > 0:
                    print("LIQUIDITY ADDED")
                    break 
                else: 
                    print("Liquidity not added")
